...
def start_processor2(pname, param=''):
    cmd0="nohup %s/%s %s >> %s/%s.log 2>&1 &" % \
        (APP_DIR, pname, param, LOG_DIR, pname.replace('/','_'))
    print('start process: ', cmd0)
    os.system(cmd0)
...
if __name__=='__main__':
    ...
    try:    
        ...
        while 1:                        
            ...
            # 检查gotak进程
            pid=get_processor_pid('%s/gotalk/gotalk' % APP_DIR)
            if pid==None:
                # 进程已死, 重启进程
                kill_processor('%s/gotalk/gotalk' % APP_DIR)
                start_processor2('gotalk/gotalk', APP_DIR)
                _ins+=1
                print("%s\tgotalk restart" % helper.time_str())
