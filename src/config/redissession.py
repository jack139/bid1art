...
class RedisStore(web.session.Store):
    ...
    """
    def __init__(self, ip='localhost', port=7480, db=0, initial_flush=False):
        self.redis_server = redis.Redis(host=ip, port=port, db=db, password='e18ffb7484f4d69c2acb40008471a71c')
        if initial_flush:
            """
