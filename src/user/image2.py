...
class handler: # 
    def POST(self):
        if helper.logged(helper.PRIV_TRD|helper.PRIV_DEL|helper.PRIV_ART):
            user_data=web.input()
            ...
            resumableFilename = user_data.get('resumableFilename')
            resumableIdentifier = user_data.get('resumableIdentifier')
            resumableType = user_data.get('resumableType')
